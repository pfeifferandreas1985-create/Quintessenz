...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  #initiated {{ position: absolute; font-size: clamp(16px, 3.4vw, 40px); font-weight: 700; letter-spacing: .34em;
                text-indent: .34em; color: var(--h); text-shadow: 0 0 24px var(--p); opacity: 0;
                white-space: nowrap; }}
  #initiated.on {{ animation: init-in 1.7s ease-out forwards; }}
  #initiated span {{ display: inline-block; }}
  #initiated.on span {{ animation: glitch .12s steps(1) 3; animation-delay: calc(var(--i) * 28ms + .35s); }}
  ...
  /* 4 x 4 soll ohne Scrollen in die Höhe passen: Kachel ist ca. 1,12-mal so hoch wie breit */
  .grid {{ display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px;
           width: min(92vw, calc((100vh - 152px) / 1.12 + 42px)); align-self: center; }}
  @media (max-aspect-ratio: 4/5) {{ .grid {{ grid-template-columns: repeat(2, 1fr); width: 92vw; }}
                                    #s-menu.on {{ overflow-y: auto; }} }}
  .tile {{ position: relative; display: block; padding: 6px 6px 26px; border: 1px solid var(--dim);
           border-radius: 6px; color: var(--p); text-decoration: none; opacity: 0; transform: scale(.4);
  ...
  .tile-sub {{ position: absolute; left: 0; right: 0; bottom: 7px; text-align: center;
               font-size: clamp(9px, .9vw, 12px); letter-spacing: .04em; opacity: .55; padding: 0 8px;
  ...
  @keyframes init-in {{ 0% {{ opacity: 0; letter-spacing: .1em; }} 18% {{ opacity: 1; letter-spacing: .34em; }}
                        78% {{ opacity: 1; }} 100% {{ opacity: 0; letter-spacing: .5em; }} }}
  @keyframes glitch {{ 0% {{ transform: translate(0,0); }} 50% {{ transform: translate(-2px,1px); opacity: .4; }}
...
(() => {{
  ...
  const wait = ms => new Promise(res => {{
    ...
    (function f(now) {{ (now - t0 >= ms || skipped) ? res() : requestAnimationFrame(f); }})(t0);
  }});
  ...
  function tippen(pre, cur, line, cps) {{
    return new Promise(res => {{
      ...
      (function f(now) {{
        const soll = Math.min(line.length, Math.floor((now - t0) / 1000 * cps));
        while (i < soll) {{
          const s = document.createElement('span'); s.className = 'g'; s.textContent = line[i++];
          pre.insertBefore(s, cur);
        }}
        (i >= line.length || skipped) ? res() : requestAnimationFrame(f);
      }})(t0);
    }});
  }}
  ...
  async function lauf() {{
    ...
    const ziel = $('#initiated');
    ziel.innerHTML = [...'WORLD RESTART INITIATED']
      .map((c, i) => `<span style="--i:${{i}}">${{c === ' ' ? '&nbsp;' : c}}</span>`).join('');
    ziel.classList.add('on');
    await wait(reduced ? 0 : 1750);
    if (skipped) return;
